...
import static com.example.i325639.myapplication.sharedelementtransition.ImageData.IMAGE_DRAWABLES;
...
public class GridAdapter extends RecyclerView.Adapter<GridAdapter.ImageViewHolder> {
  ...
  private static class ViewHolderListenerImpl implements ViewHolderListener {
    ...
    public void onLoadCompleted(ImageView view, int position) {
      ...
      if (SharedElementTransitionMainActivity.currentPosition != position) {
        return;
      }
    ...
    public void onItemClicked(View view, int position) {
      // Update the position.
      SharedElementTransitionMainActivity.currentPosition = position;

      // Exclude the clicked card from the exit transition (e.g. the card will disappear immediately
      // instead of fading out with the rest to prevent an overlapping animation of fade and move).
      ((TransitionSet) fragment.getExitTransition()).excludeTarget(view, true);

      ImageView transitioningView = (ImageView)view.findViewById(R.id.card_image);
